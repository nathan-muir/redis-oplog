...
export default class ObservableCollection {
    ...
    change(doc, modifiedFields) {
        ...
        const changedTopLevelFields = this.diffDocument(oldDoc, newDoc);
        if (!_.isEmpty(changedTopLevelFields)) {
            this.send('changed', docId, changedTopLevelFields)
        }
    }

    diffDocument(oldDoc, newDoc) {
        const fields = {};
