...
} from './lib/filterFieldsForFetching';
...
export default class ObservableCollection {
    ...
    change(doc, modifiedFields) {
        ...
        // determine which fields were removed via `$unset`
        const removedFields = [];
        modifiedFields.forEach(field => {
            const val = deepGet(doc, field);
            if (val === undefined) {
                removedFields.push(field);
            }
        });

        // merge the new doc with the one from storage
        // TODO is cloneDeep still necessary (?)
        let newDoc = deepExtend(this.store[docId], cloneDeep(doc));

        // delete the removed fields from the final doc
        // TODO compare perf with LocalCollection._compileProjection
        removedFields.forEach((field)=> {
            deepUnset(newDoc, field);
        });
        if (this.fieldsArray) {
        ...
        getTopLevelFields(modifiedFields).forEach(topLevelField => {
            changedTopLevelFields[topLevelField] = newDoc[topLevelField];
        });

        // We need to clear it again, so we don't send "restrictedField": undefined.
        if (this.fieldsArray) {
            changedTopLevelFields = this.projectFieldsOnDoc(
        ...
        this.send('changed', doc._id, changedTopLevelFields);
    }
    ...
    isLimitReached() {
        if (this.options.limit) {
            const size = _.keys(this.store).length;
